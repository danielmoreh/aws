...
public class AWS {
    ...
    public void closeResources(boolean deleteBucket) {
        if(deleteBucket)
    ...
    public boolean checkIfFileExists(String bucketName, String key) {
        try {
    ...
    public String createEC2Instance(String tagName, int numberOfInstances) {
        String ami = "ami-00e95a9222311e8ed";//the one Gal sent
        String ec2Script = "#!/bin/bash\n" +
                "mkdir testing\n" +
                "aws s3 cp s3://logim/manager.jar ./testing/manager.jar\n" +
                "cd ./testing\n" +
                "java -Xms3g -Xmx3g -jar manager.jar> log.txt\n" +
                "aws s3 cp log.txt s3://logim/log.txt\n";
        ...
        try {
            ...
            System.out.println("[DEBUG] Successfully started EC2 instance "+instanceId+" based on AMI "+ami);
    ...
    public String checkInstanceState(String instanceId) {
        DescribeInstancesRequest request = DescribeInstancesRequest.builder()
                .instanceIds(instanceId)
                .build();
    ...
    public void killAllEc2Instances() {
        DescribeInstancesResponse response = describeInstances();
        for (Reservation reservation : response.reservations()) {
            for (Instance instance : reservation.instances()) {
